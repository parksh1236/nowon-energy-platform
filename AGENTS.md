# 노원구 에너지정보플랫폼 — 작업 가이드 (Codex/Claude 공용)
...
└── platform/
    ...
    ├── carbon.json            ← 탄소중립 선도도시 기본계획 데이터 (개요·현황·목표·로드맵·사업 30개)
    └── nowon_boundary.json    ← 노원구 경계 폴리곤
```

## 화면 구성 (2개 탭)

- **⚡ 에너지정보**: 지도 + 검색 + 에너지종류/년월/용도 필터 + 통계 (기존 기능 전체)
- **🌱 탄소중립계획**: 「노원구 탄소중립 선도도시 기본계획(안)」(2026.4) 요약 —
  개요(비전·KPI), 배출현황(2016~2020 추이·부문 비중), 연차별 목표·달성률 그래프,
  단계별 추진계획(3단계), 부문별 세부사업 30개 목록. 사업 클릭 시 지도 영역에 상세 패널
  (사업총괄표 + 연차별 누적 감축목표 차트). 원본: PDF 579쪽에서 추출, carbon.json에 저장
