...
const status = doc?.querySelector('#solarStatus');
const results = doc?.querySelector('#solarResults');
const resultPanel = doc?.querySelector('#solarResultPanel');
...
function vworldKey() {
  return window.SOLAR_CONFIG?.vworldApiKey ?? 'EB2D5106-C922-3CDB-99E6-BC77DBBA1B87';
}
...
export async function initMap() {
  try {
    if (!window.ws3d?.viewer) throw new Error('VWorld 지도를 준비하는 중입니다.');
    wireMapClicks(null);
    renderMapShapes();
    if (shadowToggle?.checked) toggleShadowSimulation(true);
    setStatus('노원구 VWorld 지도에서 건물을 선택할 수 있습니다.');
...
export async function loadClimate(fetcher = fetch) {
  climatePromise ??= fetcher('./solar-data.json').then((response) => {
...
document.querySelector('#solarResultClose')?.addEventListener('click', () => { resultPanel.style.display = 'none'; });

restoreProject();
initShadowControls();
precisionOptions.hidden = new FormData(form).get('analysisMode') !== 'detailed';
window.initSolarLabMap = initMap;
}
